...
export const isAttrs = arg =>
  ...
  !Array.isArray(arg) && !(arg instanceof Node);
...
const parseKid = kid =>
  (kid instanceof Node) ? [kid] :
  Array.isArray(kid) ? parseKids(...kid) :
  typeof kid === 'string' ? [document.createTextNode(kid)] :
  typeof kid === 'number' ? [document.createTextNode(kid + '')] :
  /* else */ throwError('unrecognized child node');

const parseKids = (...kids) => kids.flatMap(parseKid);

const parseElemArgs = (...args) =>
  args.length === 0 ? [{}, []] :
  isAttrs(args[0]) ? [args[0], parseKids(...args.slice(1))] :
  /* else */ [{}, parseKids(...args)];
